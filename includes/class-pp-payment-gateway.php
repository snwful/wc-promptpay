...
class PP_Payment_Gateway extends \WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->title            = $this->get_option( 'title', __( 'PromptPay', 'woo-promptpay-n8n' ) );
        $this->description      = $this->get_option( 'description', __( 'Pay with PromptPay by scanning the QR code with your mobile banking app.', 'woo-promptpay-n8n' ) );
        ...
        // Debug logging
        error_log( 'WooPromptPay Gateway Constructor: ID = ' . $this->id );
        error_log( 'WooPromptPay Gateway Constructor: Enabled = ' . $this->enabled );
        error_log( 'WooPromptPay Gateway Constructor: Title = ' . $this->title );
        error_log( 'WooPromptPay Gateway Constructor: PromptPay ID = ' . $this->promptpay_id );
    ...
    public function is_available() {
        // Debug logging
        error_log( 'WooPromptPay Gateway: Checking availability' );
        error_log( 'WooPromptPay Gateway: Enabled = ' . $this->enabled );
        error_log( 'WooPromptPay Gateway: PromptPay ID = ' . $this->promptpay_id );
        
        // Check if gateway is enabled
        if ( 'yes' !== $this->enabled ) {
            error_log( 'WooPromptPay Gateway: Not available - disabled' );
            return false;
        }
