<?php
/**
 * WooCommerce PromptPay Payment Gateway
 * 
...
class WC_Gateway_PromptPay extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title = __( 'PromptPay', 'woo-promptpay-n8n' );
        $this->method_description = __( 'Accept payments via PromptPay QR code with slip upload verification.', 'woo-promptpay-n8n' );
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            ...
            'promptpay_id' => array(
                ...
                'type'        => 'text',
                'description' => __( 'Enter your PromptPay ID (phone number or national ID).', 'woo-promptpay-n8n' ),
                'default'     => '',
                'desc_tip'    => true,
            ),
            'n8n_webhook_url' => array(
                ...
                'type'        => 'url',
                'description' => __( 'Enter your n8n webhook URL for payment verification.', 'woo-promptpay-n8n' ),
                'default'     => '',
                'desc_tip'    => true,
            ),
            'max_attempts' => array(
    ...
    private function display_qr_code( $amount ) {
        ...
        if ( ! $qr_data ) {
            ?>
            <div class="promptpay-qr-container" style="text-align: center; margin: 20px 0; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 5px;">
                <div class="woocommerce-error">
                    <p><?php _e( 'Invalid PromptPay ID. Please contact the store administrator.', 'woo-promptpay-n8n' ); ?></p>
                </div>
            </div>
            <?php
            return;
        ...
        ?>
        <div class="promptpay-qr-container" style="text-align: center; margin: 20px 0; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 5px;">
            <h4><?php _e( 'Scan QR Code to Pay', 'woo-promptpay-n8n' ); ?></h4>
            <div class="qr-code-image" style="margin: 15px auto;">
                <img src="<?php echo esc_url( $qr_image_url ); ?>" alt="PromptPay QR Code" style="max-width: 250px; height: auto; border: 1px solid #ddd; border-radius: 5px;" />
            </div>
            <p><strong><?php printf( __( 'Amount: %s', 'woo-promptpay-n8n' ), wc_price( $amount ) ); ?></strong></p>
            <p><small><?php _e( 'Open your mobile banking app and scan this QR code to pay', 'woo-promptpay-n8n' ); ?></small></p>
        </div>
        <?php
    }
    ...
    private function display_upload_form() {
        ?>
        <div class="promptpay-upload-container" style="margin: 20px 0; padding: 20px; background: #fff; border: 1px solid #ddd; border-radius: 5px;">
            <h4><?php _e( 'Upload Payment Slip', 'woo-promptpay-n8n' ); ?></h4>
            <p><?php _e( 'After making payment, please upload your payment slip for verification:', 'woo-promptpay-n8n' ); ?></p>
            
            <div class="form-row">
                <label for="promptpay_slip"><?php _e( 'Payment Slip', 'woo-promptpay-n8n' ); ?> <span class="required">*</span></label>
                <input type="file" id="promptpay_slip" name="promptpay_slip" accept="image/*,.pdf" required />
                <small><?php _e( 'Accepted formats: JPG, PNG, PDF (Max 5MB)', 'woo-promptpay-n8n' ); ?></small>
            </div>
            
            <div class="form-row">
                <button type="button" id="upload_slip_btn" class="button" disabled><?php _e( 'Upload & Verify', 'woo-promptpay-n8n' ); ?></button>
            </div>
            
            <div id="upload_status" style="margin-top: 10px;"></div>
            
            <input type="hidden" id="payment_verified" name="payment_verified" value="0" />
        </div>
        <?php
    }
    ...
    public function validate_fields() {
        ...
        if ( empty( $_POST['payment_verified'] ) || '1' !== $_POST['payment_verified'] ) {
            wc_add_notice( __( 'Please upload and verify your payment slip before placing the order.', 'woo-promptpay-n8n' ), 'error' );
            return false;
        }
    ...
    public function process_payment( $order_id ) {
        ...
        $order->update_status( 'pending', __( 'Awaiting PromptPay payment verification', 'woo-promptpay-n8n' ) );

        // Add order note
        $order->add_order_note( __( 'PromptPay payment slip uploaded. Awaiting verification.', 'woo-promptpay-n8n' ) );
    ...
    private function display_payment_instructions( $order_id ) {
        ...
        if ( $order->has_status( 'pending' ) ) {
            ...
            <div class="woocommerce-message">
                <h3><?php _e( 'Payment Instructions', 'woo-promptpay-n8n' ); ?></h3>
                <p><?php _e( 'Your payment slip has been uploaded and is being verified. You will receive an email confirmation once the payment is approved.', 'woo-promptpay-n8n' ); ?></p>
                <p><strong><?php printf( __( 'Order Number: %s', 'woo-promptpay-n8n' ), $order->get_order_number() ); ?></strong></p>
                <p><strong><?php printf( __( 'Total Amount: %s', 'woo-promptpay-n8n' ), $order->get_formatted_order_total() ); ?></strong></p>
            </div>
            <?php
        }
    }
    ...
    public function payment_scripts() {
        ...
        // Localize script
        wp_localize_script( 'wc-promptpay-checkout', 'promptpay_params', array(
            'ajax_url' => admin_url( 'admin-ajax.php' ),
            'nonce' => wp_create_nonce( 'promptpay_upload_nonce' ),
            'messages' => array(
                'uploading' => __( 'Uploading...', 'woo-promptpay-n8n' ),
                'verifying' => __( 'Verifying payment...', 'woo-promptpay-n8n' ),
                'verified' => __( 'Payment verified successfully!', 'woo-promptpay-n8n' ),
                'error' => __( 'Upload failed. Please try again.', 'woo-promptpay-n8n' ),
            )
        ));
    ...
    public function handle_slip_upload() {
        // Verify nonce
        if ( ! wp_verify_nonce( $_POST['nonce'], 'promptpay_upload_nonce' ) ) {
            wp_send_json_error( array( 'message' => __( 'Security check failed.', 'woo-promptpay-n8n' ) ) );
        }

        // Check if file was uploaded
        if ( empty( $_FILES['payment_slip']['name'] ) ) {
            wp_send_json_error( array( 'message' => __( 'No payment slip uploaded.', 'woo-promptpay-n8n' ) ) );
        }
        ...
        if ( ! in_array( $file['type'], $allowed_types ) ) {
            wp_send_json_error( array( 'message' => __( 'Invalid file type. Please upload JPG, PNG, or PDF.', 'woo-promptpay-n8n' ) ) );
        }

        if ( $file['size'] > $max_size ) {
            wp_send_json_error( array( 'message' => __( 'File size too large. Maximum 5MB allowed.', 'woo-promptpay-n8n' ) ) );
        }
    ...
    private function verify_payment_slip( $file ) {
        ...
        if ( $is_verified ) {
            return array(
                'success' => true,
                'message' => __( 'Payment slip verified successfully.', 'woo-promptpay-n8n' )
            );
        } else {
            return array(
                'success' => false,
